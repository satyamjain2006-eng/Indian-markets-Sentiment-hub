...
Download from https://python.org/downloads — tick **"Add Python to PATH"**

### Step 2 — Open terminal in this folder
```
cd path/to/market-sentiment
```
...
```
Note: torch + transformers are large (~1.5GB). The FinBERT model (~500MB) downloads on first run only.

**Option B — Lightweight install (VADER only, much faster):**
```
pip install streamlit streamlit-autorefresh yfinance feedparser vaderSentiment pandas numpy plotly requests
```
...
## 🌐 Deploy Online for Free (Streamlit Cloud)

1. Push all 3 files to a GitHub repository
2. Go to https://streamlit.io/cloud and sign in with GitHub
3. Click New App > select your repo > select app.py > Deploy
4. Your app will be live at https://yourname-appname.streamlit.app

Note on FinBERT + Streamlit Cloud free tier: The free tier has ~1GB RAM. FinBERT needs ~1.5GB so it may not load. The app handles this gracefully and falls back to VADER automatically. For FinBERT in production, deploy on Render.com (free, more RAM).
...
## 💡 Ideas to Extend (great for CV)

- Store daily sentiment scores in SQLite and plot historical sentiment trends
- Add email/WhatsApp alert when sentiment drops sharply for a watchlist stock
- Add F&O open interest data from NSE
- Integrate Zerodha Kite Connect for true real-time prices
- Add Reddit r/IndiaInvestments sentiment scraping
- Deploy on AWS/GCP for a production-grade portfolio project
