...
with st.sidebar:
    ...
    if compare_on:
        ...
        ca_class2 = st.radio("Class B", ["📈 Index","🏅 Commodity","₿ Crypto","🇮🇳 Stock"],
                              ...
                              index=2)
        if ca_class2 == "🇮🇳 Stock":
            ca_q2 = st.text_input("Search Asset B", placeholder="e.g. TCS…",
                                   key="ca_q2", label_visibility="collapsed")
            if ca_q2:
                ca_r2 = search_companies(ca_q2, all_companies)
                for _, row in ca_r2.iterrows():
                    if st.button(f"{row['name']} [{row['symbol']}]",
                                 key=f"ca2_{row['symbol']}", use_container_width=True):
            ...
            st.session_state.ca_name2   = pick2
            st.session_state.ca_ticker2 = opts2[pick2]

        if "ca_name2" in st.session_state:
            st.caption(f"B: {st.session_state.ca_name2}")

        compare_name   = st.session_state.get("ca_name1", "📈 Nifty 50")
        compare_ticker = st.session_state.get("ca_ticker1", "^NSEI")
    st.markdown("---")
